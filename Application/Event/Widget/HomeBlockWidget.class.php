...
class HomeBlockWidget extends Controller
{
    ...
    private function assignEvent()
    {
        ...
        $type = modC('EVENT_SHOW_TYPE', 0, 'Event');//删选类型，1为后天推荐，0为全部
        ...
        if (!$list) {
            $order = $field . " " . $order;
            if ($type == 0) {
                $list = M('Event')->where(array('eTime' => array('gt',time())))->limit($num)->order($order)->select();
            } else {
                $list = M('Event')->where(array('eTime' => array('gt',time()),'is_recommend' => 1))->limit($num)->order($order)->select();
