...
function rand_follow($count,$time){
    ...
    if(empty($rand)){
        ...
        $count=$count<=($lastUid-100)?$count:($lastUid-99);
        for($k=0;$k<$count;$k++){
            $uid=rand(100,$lastUid);
            $res=D('Common/Follow')->isFollow($myuid,$uid);
            if($res){
                $k--;
            }else{
                $uids[]=$uid;
        ...
        foreach ($uids as $u){
            $rand[]=query_user(array('title', 'avatar128', 'nickname', 'uid', 'space_url', 'title', 'fans', 'following', 'rank_link', 'pos_province', 'pos_city', 'pos_district'), $u);
        }unset($u);
        foreach ($rand as &$ra){
            $ra['isfollow']=D('Common/Follow')->isFollow($myuid,$ra['uid'])?1:0;
        }
        unset($ra);
...
function friend_follow($count,$time){
    ...
    if(empty($arr)){
        ...
        $count=$count<count($friends)?$count:count($friends);
        for($k=1;$k<$count+1;$k++){
            $arr[]=query_user(array('title', 'avatar128', 'nickname', 'uid', 'space_url', 'title', 'fans', 'following', 'rank_link', 'pos_province', 'pos_city', 'pos_district'), $friends[$k]);
        }
        unset($k);
 ...
 */
function showSide($field,$type){
    $count=modC('USER_SHOW_COUNT'.$type,5,'people');
